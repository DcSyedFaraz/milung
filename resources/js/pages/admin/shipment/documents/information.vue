        ...
        <div class="container p-4" style="background-color: #14245c">
            <div class="row justify-content-between">
                ...
                <div class="col-3 my-auto">
                    <button class="btn btn-warning px-5" :disabled="submitting">
                            ...
                            >
                                <tr style="">
                                    ...
                                    <th class="text-nowrap">
                                        <span>Carton Measurement</span> <br />
    ...
    computed: {
        totalQtys() {
            return this.packinglst.reduce(
                (sum, item) => parseInt(sum) + (parseFloat(item.qty) || 0),
                0
            );
        },
        totalTotalQty() {
            return this.packinglst.reduce(
                (total, item) => total + (parseFloat(item.totalqty) || 0),
                0
            );
        },
        totalnw() {
            return this.packinglst.reduce(
                (total, item) => total + (parseFloat(item.totalnw) || 0),
                0
            );
        },
        totalgw() {
            return this.packinglst.reduce(
                (total, item) => total + (parseFloat(item.totalgw) || 0),
                0
            );
        },
        totalVolumes() {
    ...
    methods: {
        ...
        updateData(index, event, names) {
            ...
            if (this.packinglst[index]) {
                ...
                switch (names) {
                    case "qty":
                        packinglist.totalqty = this.calculateTotal(
                            packinglist.qty,
                            this.packinglst[index].orders.quantity_unit
                        );
                        break;
                    case "nw":
                    ...
                    case "wcm":
                    case "hcm":
                        packinglist.totalVolume = this.calculateVolume(
                            this.packinglst[index]
                        );
                        break;
                    default:
                        break;
        ...
        saveItem(index) {
            ...
            axios
                .post("/api/infosave/" + updatedItem.id, updatedItem)
                .then((response) => {
        ...
        calculateVolume(item) {
            ...
            if (packinglist) {
                const volume =
                    (packinglist.hcm * packinglist.wcm * packinglist.lcm) /
                    1000000;
            ...
            axios
                ...
                .catch((error) => {
                    ...
                    if (error.response && error.response.status === 422) {
                        ...
                        toastr.error(
                            "An error occurred while adding the packing_list"
                        );
                    }
                })
                .finally(() => {
